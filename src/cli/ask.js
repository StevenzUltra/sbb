...
import { EXIT, UsageError, callerIdentity, deliver, main, parse, parseDuration, previewTransport } from './util.js';
...
/**
 * The receipt inbox (task h1) is optional in M1: without it a Claude message is
 * still delivered but can only be reported as queued.
 */
async function startInbox(deps) {
  if (deps.startInbox) return deps.startInbox();
  try {
    const mod = await import('../transports/uds-inbox.js');
    return await mod.startInbox({});
  } catch {
    return undefined;
  }
}
...
export async function run(argv, deps = {}) {
  return main(async () => {
    ...
    const inbox = await startInbox(deps);
    ...
    try {
      const { receipt, text, message } = await deliver({
        ...
        send: deps.send,
        msgId: deps.msgId,
        fromSock: target.cli === 'claude' ? inbox?.sockPath : undefined,
      });
