...
| --- | --- | --- | --- |
| GET | `/api/state` | | `{ brains, tree, accounts, quota, policy, held, plans, claims, tps, teams, threads, receipts, version }` (`receipts` = newest 500 receipt-log lines, newest first; `teams[].messages` = the channel log, `threads[]` = one private thread per brain `{ id, with, title, messages }`, both as console messages, newest 300; `accounts[].clis` = every CLI the catalog can launch on that account; `recentCwds` = up to 10 working directories of live and retired brains, newest first) — one snapshot the page can render from cold |
